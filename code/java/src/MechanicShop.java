...
public class MechanicShop{
	...
	public static void AddCustomer(MechanicShop esql){//	
		...
		do {
			...
			try {
				...
				if(phone.length() <= 0 || phone.length() > 32) {
					throw new RuntimeException("Phone Number cannot be NULL or over 32 characters");	
				}
				break;
			}
			catch (Exception e) {
		...
		do {
			...
			try {
				...
				if(address.length() <= 0 || address.length() > 32) {
					throw new RuntimeException("Address cannot be NULL or over 32 characters");	
				}
				break;
			}
			catch (Exception e) {
		...
		try {	
			esql.executeUpdate("INSERT INTO Customer (id, fname, lname, phone,address) VALUES (" + id + ", \'" + fname + ", \'" + lname + ", \'" + phone + ", \'" + address + ");" );
		}catch (Exception e) {
			System.err.println (e.getMessage());
		}
	}
